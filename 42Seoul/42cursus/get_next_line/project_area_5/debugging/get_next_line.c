...
/*   Updated: 2022/07/12 17:59:58 by seoyoo           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_next_line(int fd)
{
	...
	printf("SESARCH FD MATCHING HEAD PTR START\n");
	head_ptr = search_fd_matching_head_ptr(fd, head_ptr_arr);
	printf("head_ptr address : %p\n", head_ptr);
	printf("SESARCH FD MATCHING HEAD PTR END\n");
	...
	printf("STRING MALLOCATION START\n");
	printf("head pointer starting point char : %c\n", *(head_ptr->head->start));

	str = malloc(sizeof(char) * (count_char_till_end(head_ptr) + 1));
	if (str == NULL)
		return (NULL);
	printf("STRING MALLOCATION END\n");
	printf("COPY CHAR FROM BUFFER TO STR START\n");
	copy_char_from_buffer_to_str(head_ptr, str);
	printf("COPY CHAR FROM BUFFER TO STR END\n");
	printf("TERMINATION POROTOCOL START\n");
	termination_protocol(head_ptr);
	printf("TERMINATION POROTOCOL END\n");
	printf(" ------ [ GNL END ] ------ \n\n\n\n");
	...
	while (TRUE)
	{
		if (head->end == NULL)  // when no new_line or no eof
		{
			...
			if (head->next == NULL)  // when the last char in buffer is eof -> next buffer will not be read proplerly
			{
				...
				printf("- count_char_till_end() | count : %zu\n", count);
				return (count);
			}
...
void	termination_protocol(t_h_p_node *head_ptr)
{
	...
	while (h_node->end == NULL && h_node->next != NULL)
	{
		...
		h_node = h_node->next;
		printf("\n[--Del node]\n");
		print_buffer_node_info(del_node);
		free(del_node);
	}
	...
	else
	{
		...
		printf("termination_protocol start moving from : %c\n", *(h_node->start));
		h_node->start = h_node->end + 1;
		printf("termination_protocol start movied to   : %c\n", *(h_node->start));
